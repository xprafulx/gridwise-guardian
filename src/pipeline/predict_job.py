...
if MODE == "TODAY":
    # Snap to 00:00 of the current Danish Calendar Day
    # Adding 2 hours ensures that at 00:54 AM Sunday, it snaps to April 12th.
    TARGET_DATE = (now_utc + timedelta(hours=2)).replace(hour=0, minute=0, second=0, microsecond=0)
else:
    # Snap to 00:00 of Tomorrow
...
def get_future_prices(area_name, date):
    """
    Fetches Day-Ahead prices using 'Z' suffix to force UTC interpretation.
    """
    url = "https://api.energidataservice.dk/dataset/DayAheadPrices"
    
    # 🚨 FIX: We use 'Z' for UTC and set 'end' to the start of the NEXT day
    ...
    try:
        print(f"📡 API Call: Requesting {area_name} from {start_str} to {end_str}")
        ...
        if res.status_code == 200:
            ...
            # Ensure we return exactly 24 hours
...
def generate_full_day_forecast(area_name, engine, target_date):
    ...
    for h in range(24):
        ...
        # Use fallback if a specific hour is missing
...
if __name__ == "__main__":
    run_job()        result = conn.execute(query, {"name": area_key}).fetchone()
    if not result: return None, None, None
    buffer = io.BytesIO(result[0])
    payload = joblib.load(buffer)
    return payload['model'], payload['features'], result[1]
